...
def get_min_distances(coords, types, type_map, atom2):
    ...
    atom1_idx = np.where(atomic_symbols == atom1)[0]
...
def process_structure(structure_path, atom2):
    ...
    for s in sets:
        ...
        for npy_file in npy_files:
            ...
            if coords.ndim != 2:
                print(f"Formato coordinate errato in {npy_file} {coords.shape}, salto file.")
                continue
            ...
            for frame in coords:
                all_dists.extend(get_min_distances(frame, types_expanded, type_map, atom2))
...
if __name__ == "__main__":
    ...
    for struct in structures:
        for atom2 in atom2_list:
            all_distances[atom2].extend(process_structure(struct, atom2))

    bins = np.linspace(0, 5, 100)

    # Genera grafici separati
    for atom2 in atom2_list:
        ...
        plt.plot(bin_edges[:-1], hist_smooth, label=f'{atom1}-{atom2} (n={len(all_distances[atom2])})')
        plt.xlabel("Distanza minima (Å)")
        plt.ylabel("Densità di probabilità")
        plt.title(f"Distribuzione distanza minima {atom1}-{atom2}")
        plt.legend()
        plt.grid(True)

        filename = f"Distribuzione_minima_{atom1}_{atom2}.png"
