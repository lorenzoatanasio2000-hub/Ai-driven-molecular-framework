...
def main():
    ...
    parser.add_argument("--show", action="store_true", help="Display plot interactively on screen")
    ...
    for dirpath, _, filenames in os.walk(args.dataset):
        if "type.raw" in filenames:
            structures.append(dirpath)

    print(f"Found {len(structures)} structure directories.")

    for struct in structures:
        n_atoms = count_atoms_in_structure(struct)
        if n_atoms is None or n_atoms == 0:
            continue

        sets = [d for d in os.listdir(struct) if d.startswith("set.")]
        for s in sets:
            set_path = os.path.join(struct, s)
            energy_file = os.path.join(set_path, "energy.npy")
            ...
            try:
                ...
                print(f"Error reading energies in {set_path}: {err}")
